...
def compilarIndividuos(
    directorio, listaACompilar, programa, dependencias, flagsDependencias
):
...
def test(directorioComparacion, file):
    ...
    for i in range(0, 8):
        ...
        if Cpu:
            resultCpu.append(
                float(fitness.cpuUse(optimizacionActual, Argumentos, directorioActual))
            )
        if Rob:
            robustezAux = fitness.robustness(
                optimizacionActual, Argumentos, ExecutionRobustness, directorioActual
            ).split("\n")
            resultRob.append(1.0 - float(robustezAux[0]))
        if Tiempo:
            resultTiempo.append(
                float(fitness.exTime(optimizacionActual, Argumentos, directorioActual))
            )
    if len(resultRam) <= 0:
        resultRam = [0] * 8
    else:
    ...
    result = []
    result.append(
        (" - Sin optimización <" + "-" * globalResult[0] + ">", globalResult[0])
    )
    result.append(
        (" - 00               <" + "-" * globalResult[1] + ">", globalResult[1])
    )
    result.append(
        (" - 01               <" + "-" * globalResult[2] + ">", globalResult[2])
    )
    result.append(
        (" - 02               <" + "-" * globalResult[3] + ">", globalResult[3])
    )
    result.append(
        (" - 03               <" + "-" * globalResult[4] + ">", globalResult[4])
    )
    result.append(
        (" - 0s               <" + "-" * globalResult[5] + ">", globalResult[5])
    )
    result.append(
        (" - 0fast            <" + "-" * globalResult[6] + ">", globalResult[6])
    )
    result.append(
