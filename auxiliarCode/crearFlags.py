...
def crearFlagBinaria(flag):
    data["binarias"].append({"flag": flag})
...
def crearFlagRango(flag, min, max):
    data["rango"].append({"flag": flag, "min": min, "max": max})
...
def crearFlagIntervalo(flag, intervalo):
    data["Intervalo"].append({"flag": flag, "intervalo": intervalo})
    ...
    while True:
        ...
        if tipo == -1:
            ...
            if "=" not in nombreflag:
                print(
                    "Se va a añadir un '=' al final de la flag para poder asignar el rango."
                )
            ...
            if "=" not in nombreflag:
                print(
                    "Se va a añadir un '=' al final de la flag para poder asignar el valor del intervalo elegido."
                )
                nombreflag = nombreflag + "="
